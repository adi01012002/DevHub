...
export const fetchPublicPortfolioByUsername = async (username) => {
  try {
    const response = await axios.get(`${API_BASE_URL}/portfolio/public/${username}`);
    return response.data.portfolio;
  } catch (error) {
    console.error("Error fetching public portfolio:", error.response?.data || error.message);
    throw error.response?.data || error;
  }
};

export const fetchPortfolioByUseName = async (username) => {
